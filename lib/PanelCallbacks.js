...
class PanelCallbacks {
  ...
  async assignTask(bot, userObject, options) {
    ...
    if (!isActiveTask) {
      ...
      await bot.sendMessage(
        userObject.id,
        "✅ Вам призначено нове завдання! \n📚 Переходьте в панель та приступайте до виконання!",
        {
          parse_mode: "HTML",
        }
      );
      ...
      await bot.sendMessage(
        userObject.id,
        "⚠️ Дане завдання вже було призначене раніше. \n📚 Переходьте в панель та приступайте до виконання!",
        {
          parse_mode: "HTML",
        }
      );
  ...
  async postTask(bot, groupID, options, botID) {
    try {
      ...
      const title = `#${taskTypeLabel
        .toLowerCase()
        .replaceAll(" ", "_")}\n\n<b>«${taskMatherial.title}»</b>`;
      ...
      let post = null;
      if (thumbnail) {
        post = await bot.sendPhoto(
          groupID,
          // "https://www.lingua-cloud.online/wp-content/uploads/2025/06/im_26554-linguaspace-1749984654.jpg",
          thumbnail,
          {
            caption: message,
            parse_mode: "HTML",
